...
description: "Best practices for programmatic video creation using HyperFrames, plain HTML compositions with GSAP animations rendered to MP4, with full Hebrew and RTL support. Covers composition authoring, data-* timing attributes, GSAP timeline contract, layout-before-animation methodology, visual identity gate, Hebrew fonts via Google Fonts (Heebo, Rubik, Assistant), RTL text rendering with dir=\"rtl\", Hebrew TikTok/Reels-style captions via Whisper, audio-reactive visuals, scene transitions, and bidirectional Hebrew+English text. Use when building HTML-based video content or Hebrew social/marketing videos without React. Activate for: יצירת וידאו, סרטון בעברית, וידאו שיווקי, סרטון לרשתות, כתוביות בעברית, סרטון טיקטוק, רילס, אנימציית טקסט, וידאו מ-HTML. Do NOT use for Remotion or general React video work, use remotion-best-practices for that."
license: Apache-2.0
---
