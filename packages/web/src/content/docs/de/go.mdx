...
SquadCoder Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
SquadCoder Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in SquadCoder. Du abonnierst SquadCoder Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
SquadCoder zu verwenden.
...
SquadCoder Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

SquadCoder Go funktioniert wie jeder andere Provider in SquadCoder.

1. Du meldest dich bei **<a href={console}>SquadCoder Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `SquadCoder Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann SquadCoder Go abonnieren.
:::
...
SquadCoder Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner SquadCoder Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `opencode-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben SquadCoder Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit SquadCoder zu nutzen.
