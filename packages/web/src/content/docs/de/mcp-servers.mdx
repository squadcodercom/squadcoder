...
Sie können externe Tools zu SquadCoder hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. SquadCoder unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [SquadCoder Config](https://opencode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
SquadCoder übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird SquadCoder Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von SquadCoder beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `opencode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert SquadCoder die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in SquadCoder verfügbar. Sie können sie also wie jedes andere Tool über die SquadCoder-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und SquadCoder mit Ihrem Sentry-Konto zu verbinden.
