---
title: Zen
description: 由 SquadCoder 提供的精选模型列表。
---
...
SquadCoder Zen 是由 SquadCoder 团队提供的一组经过测试和验证的模型。

:::note
SquadCoder Zen 目前处于测试版阶段。
:::

Zen 的工作方式与 SquadCoder 中的任何其他提供商相同。你登录 SquadCoder Zen 并获取 API 密钥。它是**完全可选的**，即使不用它，你也可以照常使用 SquadCoder。
...
:::tip
我们测试了一组与 SquadCoder 配合良好的精选模型和提供商。
:::
...
SquadCoder Zen 是一个 AI 网关，让你可以访问这些模型。

---

## 工作原理

SquadCoder Zen 的工作方式与 SquadCoder 中的任何其他提供商相同。

1. 登录 **<a href={console}>SquadCoder Zen</a>**，添加你的账单信息，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 SquadCoder Zen，然后粘贴你的 API 密钥。
...
在你的 SquadCoder 配置中，[模型 ID](/docs/config/#models) 使用 `opencode/<model-id>` 格式。例如，对于 GPT 5.3 Codex，你需要在配置中使用 `opencode/gpt-5.3-codex`。
...
- MiniMax M2.5 Free 目前在 SquadCoder 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Nemotron 3 Super Free 目前在 SquadCoder 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Big Pickle 是一个隐身模型，目前在 SquadCoder 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
