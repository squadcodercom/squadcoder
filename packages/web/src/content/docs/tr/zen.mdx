...
SquadCoder Zen, SquadCoder ekibi tarafından test edilip doğrulanmış modellerin bir listesidir.

:::note
SquadCoder Zen şu anda beta olarak mevcut.
:::

Zen, SquadCoder'daki diğer sağlayıcılar gibi çalışır. SquadCoder Zen'de oturum açıp API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve SquadCoder kullanmak için buna ihtiyacınız yoktur.
...
:::tip
SquadCoder ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
SquadCoder Zen, bu modellere erişmenizi sağlayan bir AI ağ geçididir.

---

## Nasıl çalışır

SquadCoder Zen, SquadCoder'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>SquadCoder Zen</a>**'de oturum açın, faturalandırma bilgilerinizi ekleyin ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, SquadCoder Zen'i seçin ve API anahtarınızı yapıştırın.
...
SquadCoder yapılandırmanızdaki [model id](/docs/config/#models) `opencode/<model-id>` biçimini kullanır. Örneğin, GPT 5.3 Codex için yapılandırmanızda `opencode/gpt-5.3-codex` kullanırsınız.
...
- MiniMax M2.5 Free, sınırlı bir süre için SquadCoder'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Nemotron 3 Super Free, sınırlı bir süre için SquadCoder'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Big Pickle, sınırlı bir süre için SquadCoder'da ücretsiz olan gizli bir modeldir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
SquadCoder Zen'i şunlar için oluşturduk:
...
4. Başka herhangi bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenme olmamasını** sağlamak. Ayrıca SquadCoder ile her zaman başka herhangi bir sağlayıcıyı da kullanabilmenizi sağlamak.
