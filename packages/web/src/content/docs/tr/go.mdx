...
SquadCoder Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
SquadCoder Go şu anda beta olarak mevcut.
:::

Go, SquadCoder'daki diğer sağlayıcılar gibi çalışır. SquadCoder Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve SquadCoder'u kullanmak için bunu kullanmanıza gerek yoktur.
...
SquadCoder Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

SquadCoder Go, SquadCoder'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>SquadCoder Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `SquadCoder Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye SquadCoder Go'ya abone olabilir.
:::
...
SquadCoder Go aşağıdaki limitleri içerir:
...
SquadCoder yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `opencode-go/kimi-k2.6` kullanmalısınız.
...
SquadCoder Go'yu şu amaçlarla oluşturduk:
...
4. SquadCoder ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
