...
SquadCoder Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

:::note
SquadCoder Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w SquadCoder. Subskrybujesz SquadCoder Go i
...
SquadCoder Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

SquadCoder Go działa jak każdy inny dostawca w SquadCoder.

1. Logujesz się do **<a href={console}>SquadCoder Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `SquadCoder Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować SquadCoder Go.
:::
...
SquadCoder Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji SquadCoder
...
Stworzyliśmy SquadCoder Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z SquadCoder.
