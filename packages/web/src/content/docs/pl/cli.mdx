---
title: CLI
description: Opcje i polecenia wiersza poleceń SquadCoder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI SquadCoder domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
opencode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać SquadCoder programowo.
...
Uruchamia interfejs terminalowy (TUI) SquadCoder.
...
Interfejs CLI SquadCoder zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami SquadCoder.
...
Dołącz terminal do już działającego serwera backendu SquadCoder uruchomionego za pomocą `serve` lub `web`.

```bash
opencode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem SquadCoder. Na przykład:
...
SquadCoder integruje się z wieloma dostawcami. Możesz użyć `opencode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Podczas uruchamiania SquadCoder ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w SquadCoder.
...
Uruchom SquadCoder w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
| `--attach`   |       | Dołącz do działającego serwera SquadCoder (np. http://localhost:4096)     |
...
Uruchom serwer SquadCoder (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
opencode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API SquadCoder bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Lista wszystkich sesji SquadCoder.
...
Pokaż statystyki wykorzystania tokenów i sesji SquadCoder.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania SquadCoder.
...
Uruchom serwer SquadCoder z interfejsem internetowym.

```bash
opencode web
```

Uruchamia to serwer HTTP i udostępnia SquadCoder przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Odinstaluj SquadCoder i usuń wszystkie powiązane pliki.
...
Aktualizuj SquadCoder do najnowszej lub określonej wersji.
...
Interfejs CLI SquadCoder przyjmuje następujące flagi globalne dla każdego polecenia.
...
SquadCoder można skonfigurować za pomocą zmiennych środowiskowych.
