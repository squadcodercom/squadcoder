...
Postoje dvije vrste agenata u SquadCoder: primarni agenti i podagenti.
...
SquadCoder dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
SquadCoder dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

SquadCoder dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Ako temperatura nije navedena, SquadCoder koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu SquadCoder konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj SquadCoder konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [SquadCoder Zen](/docs/zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
