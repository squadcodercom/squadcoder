---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u SquadCoder.
---

import config from "../../../../config.mjs"
export const console = config.console

SquadCoder koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj SquadCoder konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem SquadCoder
config.
...
SquadCoder Zen je lista modela koje je obezbedio SquadCoder tim koji su bili
testirano i potvrđeno da dobro radi sa SquadCoder. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa SquadCoder Zen.
:::
...
Radi kao i svaki drugi provajder u SquadCoder i potpuno je opcionalan za korištenje.

---

## SquadCoder Go

SquadCoder Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža SquadCoder tim i koji su testirani i verificirani da dobro rade s SquadCoder-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `SquadCoder Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s SquadCoder:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u SquadCoder.
:::
...
SquadCoder koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate SquadCoder
...
Za korištenje Google Vertex AI s SquadCoder:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki SquadCoder razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki SquadCoder razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za SquadCoder. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s SquadCoder:
...
3. Kopirajte API ključ za korištenje u SquadCoder.
...
6. **Važno**: Prije upotrebe modela oblaka u SquadCoder, morate lokalno povući informacije o modelu:
...
SquadCoder Zen je lista testiranih i verifikovanih modela koju je obezbedio SquadCoder tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>SquadCoder Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **SquadCoder Zen**.

```txt
   /connect
```

3. Unesite svoj SquadCoder API ključ.
...
Polja `limit` omogućavaju SquadCoder da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite SquadCoder konfiguraciju i:
