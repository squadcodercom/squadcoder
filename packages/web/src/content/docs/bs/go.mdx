...
SquadCoder Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
SquadCoder Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u SquadCoder-u. Pretplatite se na SquadCoder Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili SquadCoder.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa SquadCoder-om.
:::
...
SquadCoder Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

SquadCoder Go radi kao bilo koji drugi provajder u SquadCoder-u.

1. Prijavite se na **<a href={console}>SquadCoder Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `SquadCoder Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na SquadCoder Go.
:::
...
SquadCoder Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj SquadCoder konfiguraciji
...
Napravili smo SquadCoder Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz SquadCoder koristite i bilo kojeg drugog provajdera.
