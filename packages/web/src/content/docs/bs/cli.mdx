---
title: CLI
description: SquadCoder CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

SquadCoder CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa SquadCoder.
...
Pokrenite SquadCoder terminalski korisnički interfejs.
...
SquadCoder CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte SquadCoder agentima.
...
Priključite terminal na već pokrenut SquadCoder backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim SquadCoder backend-om. Na primjer:
...
SquadCoder pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se SquadCoder pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u SquadCoder.
...
Pokrenite SquadCoder u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| `--attach`   |        | Priključite na pokrenuti SquadCoder server (npr. http://localhost:4096)    |
...
Pokrenite SquadCoder headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti SquadCoder-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Upravljajte SquadCoder sesijama.
...
Navedite sve SquadCoder sesije.
...
Prikaži statistiku upotrebe tokena i troškova za vaše SquadCoder sesije.
...
Uvezite podatke sesije iz JSON datoteke ili SquadCoder dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili SquadCoder dijeljenog URL-a.
...
Pokrenite SquadCoder headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup SquadCoder-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte SquadCoder i uklonite sve povezane datoteke.
...
Ažurira SquadCoder na najnoviju verziju ili određenu verziju.
...
SquadCoder CLI prihvata sljedeće globalne zastavice.
...
SquadCoder se može konfigurirati pomoću varijabli okruženja.
