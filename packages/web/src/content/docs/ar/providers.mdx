---
title: المزوّدون
description: استخدام أي موفّر LLM في SquadCoder.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم SquadCoder ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات SquadCoder.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات SquadCoder.
...
SquadCoder Zen هي قائمة نماذج يوفّرها فريق SquadCoder وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع SquadCoder. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع SquadCoder Zen.
:::
...
يعمل مثل أي مزوّد آخر في SquadCoder واستخدامه اختياري بالكامل.

---

## SquadCoder Go

SquadCoder Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق SquadCoder، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع SquadCoder.

1. شغّل الأمر `/connect` في TUI، واختر `SquadCoder Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع SquadCoder:
...
:::info
استخدام اشتراك Claude Pro/Max داخل SquadCoder غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم SquadCoder نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر SquadCoder
...
لاستخدام Google Vertex AI مع SquadCoder:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة SquadCoder تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة SquadCoder مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع SquadCoder. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع SquadCoder:
...
3. انسخ مفتاح API لاستخدامه في SquadCoder.
...
6. **مهم**: قبل استخدام نماذج السحابة في SquadCoder، يجب سحب معلومات النموذج محليا:
...
SquadCoder Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق SquadCoder. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>SquadCoder Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **SquadCoder Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ SquadCoder.
...
تسمح حقول `limit` لـ SquadCoder بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
