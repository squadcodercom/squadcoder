...
SquadCoder Go er et lavkostnadsabonnement — **$5 for din første måned**, deretter **$10/måned** — som gir deg pålitelig tilgang til populære åpne kodemodeller.

:::note
SquadCoder Go er for øyeblikket i beta.
:::

Go fungerer som enhver annen leverandør i SquadCoder. Du abonnerer på SquadCoder Go og
...
SquadCoder Go gir deg tilgang til disse modellene for **$5 for din første måned**, deretter **$10/måned**.

---

## Hvordan det fungerer

SquadCoder Go fungerer som enhver annen leverandør i SquadCoder.

1. Du logger inn på **<a href={console}>SquadCoder Zen</a>**, abonnerer på Go, og
   kopierer din API-nøkkel.
2. Du kjører kommandoen `/connect` i TUI-en, velger `SquadCoder Go`, og limer inn
   din API-nøkkel.
3. Kjør `/models` i TUI-en for å se listen over modeller som er tilgjengelige gjennom Go.

:::note
Kun ett medlem per arbeidsområde kan abonnere på SquadCoder Go.
:::
...
SquadCoder Go inkluderer følgende grenser:
...
[Modell-ID-en](/docs/config/#models) i din SquadCoder-konfigurasjon
...
Vi opprettet SquadCoder Go for å:
...
4. Ha **ingen innelåsing** ved å la deg bruke en hvilken som helst annen leverandør med SquadCoder også.
