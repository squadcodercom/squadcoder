...
SquadCoder Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
SquadCoder Go is currently in beta.
:::

Go works like any other provider in SquadCoder. You subscribe to SquadCoder Go and
...
SquadCoder Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

SquadCoder Go works like any other provider in SquadCoder.

1. You sign in to **<a href={console}>SquadCoder Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `SquadCoder Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to SquadCoder Go.
:::
...
SquadCoder Go includes the following limits:
...
4. Have **no lock-in** by allowing you to use any other provider with SquadCoder as well.
