---
title: Zen
description: 由 SquadCoder 提供的精選模型列表。
---
...
SquadCoder Zen 是由 SquadCoder 團隊提供、經過測試與驗證的模型清單。

:::note
SquadCoder Zen 目前仍處於 beta。
:::

Zen 的運作方式和 SquadCoder 中的其他供應商一樣。你登入 SquadCoder Zen 並取得
...
:::tip
我們測試了一組與 SquadCoder 搭配良好的精選模型與供應商。
:::
...
SquadCoder Zen 是一個 AI 閘道，讓你可以存取這些模型。

---

## 運作方式

SquadCoder Zen 的運作方式和 SquadCoder 中的其他供應商一樣。

1. 你登入 **<a href={console}>SquadCoder Zen</a>**，加入帳單資訊，然後複製你的 API 金鑰。
2. 你在 TUI 中執行 `/connect` 指令，選擇 SquadCoder Zen，然後貼上你的 API 金鑰。
...
SquadCoder 設定中的 [模型 ID](/docs/config/#models) 會使用 `opencode/<model-id>`
...
- MiniMax M2.5 Free 在 SquadCoder 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3 Super Free 在 SquadCoder 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個隱身模型，在 SquadCoder 上限時免費提供。團隊正在利用這段時間收集回饋並改進模型。
