...
Para depurar problemas con SquadCoder, comience verificando los registros y los datos locales que almacena en el disco.
...
SquadCoder Desktop ejecuta un servidor SquadCoder local (el sidecar `opencode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `SquadCoder` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
SquadCoder también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que SquadCoder pueda reconstruirlo.

1. Salga de SquadCoder Desktop por completo.
...
3. Reinicie el escritorio SquadCoder.

---

### Solucionar problemas de conexión del servidor

SquadCoder Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
En Windows, el escritorio SquadCoder requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de SquadCoder.

---

### Notificaciones que no se muestran

SquadCoder Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para SquadCoder en la configuración de su sistema operativo, y
...
1. Salga del escritorio SquadCoder.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio SquadCoder):
...
Si tiene problemas con SquadCoder:
