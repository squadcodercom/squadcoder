---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di SquadCoder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

SquadCoder fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire SquadCoder avvia la TUI per la directory corrente.
...
Quando usi la TUI di SquadCoder, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a SquadCoder. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da SquadCoder. _Alias_: `/quit`, `/q`
