---
title: Provider
description: Usa qualsiasi provider LLM in SquadCoder.
---

import config from "../../../../config.mjs"
export const console = config.console

SquadCoder usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di SquadCoder.
...
SquadCoder Zen e una lista di modelli forniti dal team SquadCoder, testati e
verificati per funzionare bene con SquadCoder. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da SquadCoder Zen.
:::
...
Funziona come qualsiasi altro provider in SquadCoder ed e totalmente opzionale.
...
Per usare Amazon Bedrock con SquadCoder:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in SquadCoder non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
SquadCoder usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare SquadCoder
...
Per usare Google Vertex AI con SquadCoder:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di SquadCoder come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di SquadCoder elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per SquadCoder. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con SquadCoder:
...
3. Copia la chiave API da usare in SquadCoder.
...
6. **Importante**: Prima di usare modelli cloud in SquadCoder, devi scaricare le informazioni del modello localmente:
...
SquadCoder Zen è una lista di modelli testati e verificati forniti dal team SquadCoder. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>SquadCoder Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **SquadCoder Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di SquadCoder.
...
I campi `limit` permettono a SquadCoder di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
