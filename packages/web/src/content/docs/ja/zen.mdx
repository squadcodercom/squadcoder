---
title: SquadCoder Zen
description: SquadCoder によって提供されるモデルの厳選されたリスト。
---
...
SquadCoder Zen は、SquadCoder チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

:::note
SquadCoder Zen は現在ベータ版です。
:::

Zen は SquadCoder のほかのプロバイダーと同じように動作します。SquadCoder Zen にログインして API キーを取得できます。これは **完全にオプション** であり、SquadCoder を使うために必須ではありません。
...
:::tip
SquadCoder でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
SquadCoder Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

SquadCoder Zen は、SquadCoder のほかのプロバイダーと同じように動作します。

1. **<a href={console}>SquadCoder Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、SquadCoder Zen を選んで API キーを貼り付けます。
...
SquadCoder 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.3 Codex では設定に `opencode/gpt-5.3-codex` を使用します。
...
- MiniMax M2.5 Free は期間限定で SquadCoder で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Super Free は期間限定で SquadCoder で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で SquadCoder で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
私たちが SquadCoder Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに SquadCoder では常にほかのプロバイダーも使えるようにすること。
