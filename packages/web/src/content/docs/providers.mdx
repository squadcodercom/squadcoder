---
title: Providers
description: Using any LLM provider in SquadCoder.
---

import config from "../../../config.mjs"
export const console = config.console

SquadCoder uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your SquadCoder config.
...
You can customize the providers through the `provider` section in your SquadCoder
config.
...
SquadCoder Zen is a list of models provided by the SquadCoder team that have been
tested and verified to work well with SquadCoder. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with SquadCoder Zen.
:::

1. Run the `/connect` command in the TUI, select `SquadCoder Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in SquadCoder and is completely optional to use.

---

## SquadCoder Go

SquadCoder Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the SquadCoder team that have been
tested and verified to work well with SquadCoder.

1. Run the `/connect` command in the TUI, select `SquadCoder Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with SquadCoder:
...
SquadCoder. Anthropic explicitly prohibits this.

Previous versions of SquadCoder came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in SquadCoder with zero setup:
...
GitLab Duo support in SquadCoder is experimental. Features, configuration, and
behavior may change in future releases.
:::

SquadCoder integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
SquadCoder uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock SquadCoder
...
When a `duo-workflow-*` model is selected, SquadCoder will:
...
4. Route tool execution requests through SquadCoder's permission-gated tool system
...
To use Google Vertex AI with SquadCoder:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each SquadCoder conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each SquadCoder conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for SquadCoder. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with SquadCoder:
...
3. Copy the API key for use in SquadCoder.
...
6. **Important**: Before using cloud models in SquadCoder, you must pull the model information locally:
...
SquadCoder Zen is a list of tested and verified models provided by the SquadCoder team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>SquadCoder Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **SquadCoder Zen**.

   ```txt
   /connect
   ```

3. Enter your SquadCoder API key.
...
The `limit` fields allow SquadCoder to understand how much context you have left. Standard providers pull these from models.dev automatically.
